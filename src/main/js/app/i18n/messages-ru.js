// RU messages
export default {
    //NavBar
    'app.react.start': 'Реакт старт',
    'app.rest.title': 'Рест',
    'app.organization.title': 'Организации',
    'app.state.title': 'Стейт',
    'app.webSocket.title': 'Вебсокеты',
    'app.upload.title': 'Загрузка',
    'app.home.title': 'Главная',
    'app.language': 'Язык',

    // Page layout
    'app.page.main.text': 'Базовая конфигурация проекта с использованием React + Redux + Bootstrap.',
    'app.page.state.text': 'Чтение и запись данных в стейт приложения.',
    'app.page.upload.text': 'Загрузка файлов на сервер.',
    'app.page.rest.text.first': 'Получаем с сервера данные из базы данных таблицы пользователя.',
    'app.page.rest.text.second': 'Реализована возможность добавления и удаления пользователей.',
    'app.page.organization.text.first': 'Получаем с сервера данные из базы данных таблицы организаций.',
    'app.page.organization.text.second': 'Реализована возможность добавления и удаления организации.',
    'app.page.websocket.text': 'Обмен сообщениями в стиле WebSocket включая использование STOMP в качестве суб-протокола.',

    // Page Users
    'app.user.table.title': 'Пользователи',
    'app.user.table.id': 'ID',
    'app.user.table.name': 'Имя',
    'app.user.table.role': 'Роль',
    'app.user.table.password': 'Пароль',
    'app.user.table.delete': 'Удалить',
    'app.user.table.add': 'Добавить пользователя',

    // Page Organizations
    'app.organization.table.title': 'Организация',
    'app.organization.table.id': 'ID',
    'app.organization.table.name': 'Наименование',
    'app.organization.table.form': 'ОПФ',
    'app.organization.table.capital': 'Уставной капитал',
    'app.organization.table.delete': 'Удалить',
    'app.organization.table.add': 'Добавить организацию',

    // Page Websocket
    'app.websocket.input.message': 'Введите сообщение',
    'app.websocket.button.send': 'Отправить сообщение',

    // Page upload
    'app.upload.input.message': 'Загрузите изображение',
};
